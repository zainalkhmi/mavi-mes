...
import { getAllDrawings, saveDrawing, deleteDrawing, saveLiveMeasurement } from '../utils/supabaseUtilityDB';
...
export default function DrawingManager() {
    ...
    const renderLabelBadge = (dim, index, color, labelText, isActive) => {
        ...
        if (isBalloonMode) {
            const balloonRadius = 14;
            return (
                <g style={{ cursor: 'pointer' }} onClick={(e) => { e.stopPropagation(); setActiveDimId(dim.id); }}>
                    {isActive && (
                        <circle cx={lx} cy={ly} r={balloonRadius + 6} fill="none" stroke={color} strokeWidth="1.5" opacity="0.8">
                            <animate attributeName="r" values={`${balloonRadius + 2};${balloonRadius + 12};${balloonRadius + 2}`} dur="2s" repeatCount="indefinite" />
                    ...
                    <circle cx={lx + 10} cy={ly - 10} r="5.5" fill="#0f172a" stroke="#ffffff" strokeWidth="1" />
                    <text x={lx + 10} y={ly - 7.5} textAnchor="middle" fill={catColor} fontSize="7" fontWeight="bold">
            ...
            if (indicatorType === 'horizontal') {
                ...
                return (
                    <g key={dim.id} style={{ cursor: 'pointer' }} onClick={(e) => { e.stopPropagation(); setActiveDimId(dim.id); }}>
                        ...
                        {renderLabelBadge(dim, idx, color, `∠ ${dim.spec}°`, isActive)}
                    </g>
                );
                ...
                return (
                    <g key={dim.id} style={{ cursor: 'pointer' }} onClick={(e) => { e.stopPropagation(); setActiveDimId(dim.id); }}>
                        ...
                        {renderLabelBadge(dim, idx, color, `▢ ${dim.spec} {dim.unit}`, isActive)}
                    </g>
                );
                ...
                return (
                    <g key={dim.id} style={{ cursor: 'pointer' }} onClick={(e) => { e.stopPropagation(); setActiveDimId(dim.id); }}>
                        ...
                        {renderLabelBadge(dim, idx, color, `${symbolChar} ${dim.spec} ${dim.unit}`, isActive)}
                    </g>
                );
            } else {
