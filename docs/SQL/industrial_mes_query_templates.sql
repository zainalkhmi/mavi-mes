-- ============================================================================
-- MAVICORE MES: MASTER INDUSTRIAL SQL QUERY TEMPLATES (PostgreSQL / Supabase)
-- Standard manufacturing queries for ISA-95 Manufacturing Execution System
-- Tables: work_orders, production_logs, machines_equipment, downtime_events,
--         maintenance_orders, qa_checksheets, defect_records, inventory_parts,
--         material_movements, bill_of_materials, ehs_incidents
-- ============================================================================
...
-- 1.1 Work Order Progress & Completion Rate
-- Menghitung persentase progres penyelesaian SPK dan mendeteksi pesanan yang mendekati deadline
SELECT 
...
-- 1.2 Performa Operator & First Pass Yield
-- Menghitung total output barang bagus, barang scrap, yield rate (%), dan rata-rata cycle time per operator
SELECT 
...
-- 1.3 Tren Output Produksi per Jam (Hourly Production Output)
SELECT 
...
-- 2.1 Status Mesin & OEE vs Target (Health Check)
SELECT 
...
-- 2.2 Pareto Downtime per Kategori & Mesin
SELECT 
...
-- 2.3 Outstanding Maintenance Work Orders (Preventive vs Corrective)
SELECT 
...
-- ----------------------------------------------------------------------------
-- 3. QUALITY ASSURANCE & DEFECTS
-- ----------------------------------------------------------------------------

-- 3.1 Tingkat Kelulusan QC (Pass Rate per Part)
SELECT 
...
-- 3.2 Pareto Cacat Produk & Tindakan Koreksi (Root Cause & CAPA)
SELECT 
...
-- ----------------------------------------------------------------------------
-- 4. WAREHOUSE, INVENTORY & BOM
-- ----------------------------------------------------------------------------

-- 4.1 Peringatan Stok Kritis (Under Reorder Point)
SELECT 
...
-- 4.2 Rekapitulasi Mutasi Material (Inbound vs Outbound Issue)
SELECT 
  part_number AS "Part Number",
  movement_type AS "Jenis Mutasi",
  SUM(quantity) AS "Total Kuantitas",
  COUNT(movement_id) AS "Jumlah Transaksi",
  MAX(timestamp) AS "Mutasi Terakhir"
FROM material_movements
GROUP BY part_number, movement_type
ORDER BY part_number, movement_type;

-- 4.3 Struktur BOM & Estimasi Biaya Komponen
SELECT 
...
-- ----------------------------------------------------------------------------
-- 5. MULTI-TABLE JOINS & K3 EHS
-- ----------------------------------------------------------------------------

-- 5.1 Multi-Table JOIN: SPK ──► Scan Operator ──► Defect
SELECT 
...
-- 5.2 Ringkasan Laporan Keselamatan Kerja (K3 EHS)
SELECT 
